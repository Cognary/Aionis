...
async function startMockServer() {
  ...
  const server = createServer(async (req, res) => {
    ...
    if (req.url === "/v1/memory/replay/runs/get") {
      res.writeHead(200, { "content-type": "application/json", "x-request-id": "req_recover" });
...
test("replay recover returns inspection-only recovery plan", async (t) => {
  ...
  assert.equal(payload.command, "aionis replay recover");
  assert.equal(payload.data.run_id, "run_ok");
  assert.equal(payload.data.recovery.recoverable, true);
  assert.equal(payload.data.recovery.recovery_path, "compile_then_candidate_or_dispatch");
  assert.equal(payload.data.recovery.blocker_count, 0);

  assert.equal(mock.requests.length, 1);
  assert.equal(mock.requests[0].url, "/v1/memory/replay/runs/get");
  assert.equal(mock.requests[0].body.run_id, "run_ok");
