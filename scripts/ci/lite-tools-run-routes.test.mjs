...
test("lite tools/run reads lifecycle from sqlite decision store", () => {
  const out = runSnippet(`
    ...
    const main = async () => {
      const tmpDir = mkdtempSync(path.join(os.tmpdir(), "aionis-lite-tools-run-"));
      ...
      try {
        await liteWriteStore.withTx(async () => {
          const commitId = await liteWriteStore.insertCommit({
            scope: "default",
            parentCommitId: null,
            inputSha256: "lite-tools-run-sha",
            diffJson: JSON.stringify({ nodes: [], edges: [] }),
            actor: "lite-tools-run-test",
            modelVersion: null,
            promptVersion: null,
            commitHash: "lite-tools-run-commit",
          });
          ...
          await liteWriteStore.insertRuleDef({
            ruleNodeId: "52000000-0000-0000-0000-000000000001",
            scope: "default",
        ...
        registerMemoryFeedbackToolRoutes({
          ...
          store: {
            withTx: async () => { throw new Error("lite tools/run should not use store.withTx"); },
            withClient: async () => { throw new Error("lite tools/run should not use store.withClient"); },
          },
        ...
        try {
          ...
          process.stdout.write("__RESULT__" + JSON.stringify({
            ...
            runBody: JSON.parse(runRes.body),
          }));
        } finally {
          await app.close();
        }
  ...
  assert.equal(parsed.runBody.lifecycle.status, "decision_recorded");
  assert.equal(parsed.runBody.lifecycle.decision_count, 1);
  assert.equal(parsed.runBody.feedback.total, 0);
  assert.equal(parsed.runBody.feedback.tools_feedback_count, 0);
  assert.equal(parsed.runBody.feedback.linked_decision_count, 0);
  assert.equal(parsed.runBody.decisions.length, 1);
  assert.equal(parsed.runBody.decisions[0].selected_tool, "kubectl");
  assert.equal(parsed.runBody.lifecycle_summary.decision_count, 1);
});
