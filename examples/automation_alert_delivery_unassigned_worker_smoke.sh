...
TENANT_ID="${TENANT_ID:-aaduw_t_$(date +%s)_$(uuidgen | tr '[:upper:]' '[:lower:]' | cut -d- -f1)}"
SCOPE="${SCOPE:-aaduw_s_$(date +%s)_$(uuidgen | tr '[:upper:]' '[:lower:]' | cut -d- -f1)}"
AUTOMATION_ID_A="${AUTOMATION_ID_A:-aaduw_a_$(date +%s)}"
AUTOMATION_ID_B="${AUTOMATION_ID_B:-aaduw_b_$(date +%s)}"
ROUTE_LABEL="${ROUTE_LABEL:-aaduw-smoke}"
...
if [[ -z "${ADMIN_TOKEN}" ]]; then
  echo "automation alert delivery unassigned worker smoke requires ADMIN_TOKEN or AIONIS_ADMIN_TOKEN" >&2
  exit 1
fi
...
create_automation() {
  ...
  post_json "/v1/automations/create" "$(
    jq -cn --arg tenant "$TENANT_ID" --arg scope "$SCOPE" --arg automation_id "$automation_id" --arg playbook_id "$playbook_id" \
      '{tenant_id:$tenant,scope:$scope,actor:"automation_alert_delivery_unassigned_worker_smoke",automation_id:$automation_id,name:$automation_id,status:"active",graph:{nodes:[{node_id:"deploy",kind:"playbook",playbook_id:$playbook_id,mode:"strict"}],edges:[]}}'
  )" >/dev/null
}
...
run_automation() {
  ...
  post_json "/v1/automations/run" "$(
    jq -cn --arg tenant "$TENANT_ID" --arg scope "$SCOPE" --arg automation_id "$automation_id" \
      '{tenant_id:$tenant,scope:$scope,actor:"automation_alert_delivery_unassigned_worker_smoke",automation_id:$automation_id,params:{allow_local_exec:true,execution_backend:"local_process",allowed_commands:["uname"]}}'
  )" >/dev/null
}
...
if [[ "$sandbox_enabled" != "true" || "$sandbox_mode" != "local_process" ]]; then
  echo "automation alert delivery unassigned worker smoke requires sandbox.enabled=true and sandbox.mode=local_process" >&2
...
post_admin_json "/v1/admin/control/tenants" "$(
  jq -cn --arg tenant "$TENANT_ID" '{tenant_id:$tenant,display_name:("Automation Alert Unassigned Worker Smoke " + $tenant),status:"active"}'
...
route_json="$(post_admin_json "/v1/admin/control/alerts/routes" "$(
  jq -cn --arg tenant "$TENANT_ID" --arg target "$FAILING_TARGET" --arg label "$ROUTE_LABEL" \
    '{tenant_id:$tenant,channel:"webhook",target:$target,label:$label,events:["automation.slo.success_rate"],metadata:{automation_dispatch_policy:{retry_max_attempts:1}}}'
)" )"
...
compile_json="$(record_and_compile_playbook "Automation Alert Unassigned Worker Playbook" "whoami" '[]' "alert-delivery-unassigned-worker-recorded" "$PLAYBOOK_RUN_ID")"
...
echo
echo "== assign one owned backlog item and one unassigned backlog item =="
post_admin_json "/v1/admin/control/alerts/deliveries/assign" "$(
  jq -cn --arg id "$FAILED_A_ID" '{ids:[$id],owner:"ops-oncall",workflow_state:"replay_backlog",note:"owned backlog item"}'
)" | jq '{ok, updated}'
post_admin_json "/v1/admin/control/alerts/deliveries/assign" "$(
  jq -cn --arg id "$FAILED_B_ID" '{ids:[$id],workflow_state:"replay_backlog",note:"unassigned backlog item"}'
)" | jq '{ok, updated}'

echo
echo "== unassigned worker dry-run should only match unassigned item =="
dry_json="$(AIONIS_BASE_URL="$BASE_URL" AIONIS_ADMIN_TOKEN="$ADMIN_TOKEN" npm run -s job:hosted-alert-delivery-unassigned -- --once --tenant-id "$TENANT_ID" --override-target "$RECOVERY_TARGET" --dry-run --limit 10)"
echo "$dry_json" | jq '{ok, owner_mode, backlog, matched, dry_run_rows, ids}'
dry_matched="$(echo "$dry_json" | jq -r '.matched')"
dry_first_id="$(echo "$dry_json" | jq -r '.ids[0] // ""')"
if [[ "$dry_matched" != "1" || "$dry_first_id" != "$FAILED_B_ID" ]]; then
  echo "expected unassigned worker dry-run to match only unassigned replay-backlog delivery" >&2
  exit 1
fi

echo
echo "== unassigned worker live run should replay only unassigned item =="
live_json="$(AIONIS_BASE_URL="$BASE_URL" AIONIS_ADMIN_TOKEN="$ADMIN_TOKEN" npm run -s job:hosted-alert-delivery-unassigned -- --once --tenant-id "$TENANT_ID" --override-target "$RECOVERY_TARGET" --limit 10)"
echo "$live_json" | jq '{ok, owner_mode, backlog, matched, replayed, failed, skipped, ids}'
live_replayed="$(echo "$live_json" | jq -r '.replayed')"
if [[ "$live_replayed" != "1" ]]; then
  echo "expected unassigned worker to replay one unassigned delivery" >&2
  exit 1
fi

echo
echo "== verify only unassigned item has replay row =="
...
echo "$deliveries_all_json" | jq '{ok, deliveries:(.deliveries | map({delivery_id, status, automation_id:.metadata.automation_id, replay_of_delivery_id:(.metadata.replay_of_delivery_id // null), owner:(.metadata.alert_workflow.owner // null), workflow_state:(.metadata.alert_workflow.state // null)}))}'
...
if [[ "$replayed_a_count" != "0" || "$replayed_b_count" != "1" ]]; then
  echo "expected only unassigned failed delivery to be replayed" >&2
  exit 1
fi

echo
echo "ok: automation_alert_delivery_unassigned_worker_smoke completed"
