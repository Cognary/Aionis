...
SCOPE="${SCOPE:-automation_comp_db_failure_$(date +%s)_$(uuidgen | tr '[:upper:]' '[:lower:]' | cut -d- -f1)}"
...
if [[ "$sandbox_enabled" != "true" || "$sandbox_mode" != "local_process" ]]; then
  echo "automation compensation db failure smoke requires sandbox.enabled=true and sandbox.mode=local_process" >&2
...
COMP_RUN_ID="$(lower_uuid)"
AUTOMATION_ID="automation_comp_db_failure_$(date +%s)"

echo
echo "== compile success, repair-gated, and successful compensation playbooks =="
...
comp_compile_json="$(record_and_compile_playbook "Destroy Env Playbook" "auto_ok" "uname" '["-m"]' "destroy-env-ok" "$COMP_RUN_ID")"
...
COMP_PLAYBOOK_ID="$(echo "$comp_compile_json" | jq -r '.playbook_id')"
...
echo "$comp_compile_json" | jq '{comp_playbook_id:.playbook_id, comp_status:.status}'

echo
echo "== create automation with compensation policy =="
create_json="$(post_json "/v1/automations/create" "$(
  jq -cn \
    ...
    '{
      tenant_id:$tenant,
      scope:$scope,
      actor:"automation_compensation_db_failure_smoke",
      automation_id:$automation_id,
      name:"Automation Compensation DB Failure Smoke",
      status:"active",
      graph:{
        nodes:[
          {
...
echo
echo "== reject repair (expect compensation finalize db failure to fail-closed) =="
reject_json="$(post_json "/v1/automations/runs/reject_repair" "$(
  jq -cn --arg tenant "$TENANT_ID" --arg scope "$SCOPE" --arg run_id "$run_id" \
    '{
      ...
      actor:"automation_compensation_db_failure_smoke",
      run_id:$run_id,
      reason:"trigger compensation finalize db failure"
    }'
)")"
echo "$reject_json" | jq '{
  run:{
...
reject_state="$(echo "$reject_json" | jq -r '.run.lifecycle_state')"
...
reject_root_cause="$(echo "$reject_json" | jq -r '.run.root_cause_code')"
install_env_outcome="$(echo "$reject_json" | jq -r '.nodes[] | select(.node_id=="install_env") | .terminal_outcome')"
if [[ "$reject_state" != "terminal" || "$reject_outcome" != "failed" ]]; then
  echo "expected compensation finalize db failure to end terminal/failed, got state=${reject_state} outcome=${reject_outcome}" >&2
  exit 1
fi
if [[ "$reject_comp_status" != "failed" || "$reject_root_cause" != "automation_injected_db_failure" ]]; then
  echo "expected compensation_status=failed and root_cause=automation_injected_db_failure, got comp=${reject_comp_status} root=${reject_root_cause}" >&2
  exit 1
fi
if [[ "$install_env_outcome" != "compensated" ]]; then
  echo "expected successful compensation work to remain recorded on install_env, got ${install_env_outcome}" >&2
  exit 1
fi
...
echo "$get_json" | jq '{run:{lifecycle_state:.run.lifecycle_state, terminal_outcome:.run.terminal_outcome, compensation_status:.run.compensation_status, root_cause_code:.run.root_cause_code}, nodes:(.nodes | map({node_id, terminal_outcome, compensation_status}))}'
get_state="$(echo "$get_json" | jq -r '.run.lifecycle_state')"
if [[ "$get_state" != "terminal" ]]; then
  echo "expected persisted run state to remain terminal after compensation db failure, got ${get_state}" >&2
  exit 1
fi

echo
echo "ok: automation_compensation_db_failure_smoke completed"
