...
TENANT_ID="${TENANT_ID:-automation_alert_delivery_replay_tenant_$(date +%s)_$(uuidgen | tr '[:upper:]' '[:lower:]' | cut -d- -f1)}"
SCOPE="${SCOPE:-automation_alert_delivery_replay_smoke_$(date +%s)_$(uuidgen | tr '[:upper:]' '[:lower:]' | cut -d- -f1)}"
AUTOMATION_ID="${AUTOMATION_ID:-automation_alert_delivery_replay_smoke_$(date +%s)}"
ROUTE_LABEL="${ROUTE_LABEL:-automation-alert-replay-smoke}"
...
if [[ -z "${ADMIN_TOKEN}" ]]; then
  echo "automation alert delivery replay smoke requires ADMIN_TOKEN or AIONIS_ADMIN_TOKEN" >&2
  exit 1
fi
...
if [[ "$sandbox_enabled" != "true" || "$sandbox_mode" != "local_process" ]]; then
  echo "automation alert delivery replay smoke requires sandbox.enabled=true and sandbox.mode=local_process" >&2
...
post_admin_json "/v1/admin/control/tenants" "$(
  jq -cn --arg tenant "$TENANT_ID" \
    '{tenant_id:$tenant,display_name:("Automation Alert Replay Smoke " + $tenant),status:"active"}'
...
compile_json="$(record_and_compile_playbook "Automation Alert Delivery Replay Playbook" "whoami" '[]' "alert-delivery-replay-recorded" "$PLAYBOOK_RUN_ID")"
...
create_json="$(post_json "/v1/automations/create" "$(
  jq -cn --arg tenant "$TENANT_ID" --arg scope "$SCOPE" --arg automation_id "$AUTOMATION_ID" --arg playbook_id "$PLAYBOOK_ID" \
    '{
      tenant_id:$tenant,
      scope:$scope,
      actor:"automation_alert_delivery_replay_smoke",
      automation_id:$automation_id,
      name:"Automation Alert Delivery Replay Smoke",
      status:"active",
...
[[ "$(echo "$run_json" | jq -r '.run.terminal_outcome')" == "failed" ]] || { echo "expected failed automation run" >&2; exit 1; }
...
[[ "$(echo "$dispatch_fail_json" | jq -r '.failed')" == "1" ]] || { echo "expected one failed delivery" >&2; exit 1; }
...
echo "$deliveries_failed_json" | jq '{ok, deliveries:(.deliveries | map({delivery_id, id, route_id, status, response_code, error, has_payload:(.metadata.payload_snapshot != null)}))}'
[[ -n "$FAILED_DELIVERY_ID" && "$FAILED_DELIVERY_ID" != "null" ]] || { echo "expected failed delivery id" >&2; exit 1; }

echo
echo "== replay failed delivery to recovery target =="
replay_json="$(post_admin_json "/v1/admin/control/alerts/deliveries/replay" "$(
  jq -cn --arg id "$FAILED_DELIVERY_ID" --arg recovery_target "$RECOVERY_TARGET" \
    '{ids:[$id],dry_run:false,override_target:$recovery_target}'
)")"
echo "$replay_json" | jq '{dry_run, replayed, failed, skipped, results:(.results | map({delivery_id, replay_of_delivery_id, route_id, status, response_code, error}))}'
[[ "$(echo "$replay_json" | jq -r '.replayed')" == "1" ]] || { echo "expected replay to send one delivery" >&2; exit 1; }

echo
echo "== deliveries after replay =="
...
echo "$deliveries_all_json" | jq '{ok, deliveries:(.deliveries | map({id, route_id, status, response_code, error, replay_of_delivery_id:(.metadata.replay_of_delivery_id // null)}))}'
...
[[ "$replayed_sent_count" == "1" ]] || { echo "expected one sent replay delivery row" >&2; exit 1; }

echo
echo "ok: automation_alert_delivery_replay_smoke completed"
