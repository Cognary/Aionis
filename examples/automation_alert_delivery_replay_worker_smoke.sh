...
TENANT_ID="${TENANT_ID:-adrw_t_$(date +%s)_$(uuidgen | tr '[:upper:]' '[:lower:]' | cut -d- -f1)}"
SCOPE="${SCOPE:-adrw_s_$(date +%s)_$(uuidgen | tr '[:upper:]' '[:lower:]' | cut -d- -f1)}"
AUTOMATION_ID="${AUTOMATION_ID:-adrw_$(date +%s)}"
ROUTE_LABEL="${ROUTE_LABEL:-adrw-smoke}"
...
if [[ -z "${ADMIN_TOKEN}" ]]; then
  echo "automation alert delivery replay worker smoke requires ADMIN_TOKEN or AIONIS_ADMIN_TOKEN" >&2
  exit 1
fi
...
if [[ "$sandbox_enabled" != "true" || "$sandbox_mode" != "local_process" ]]; then
  echo "automation alert delivery replay worker smoke requires sandbox.enabled=true and sandbox.mode=local_process" >&2
...
post_admin_json "/v1/admin/control/tenants" "$(
  jq -cn --arg tenant "$TENANT_ID" \
    '{tenant_id:$tenant,display_name:("Automation Alert Replay Worker Smoke " + $tenant),status:"active"}'
...
compile_json="$(record_and_compile_playbook "Automation Alert Delivery Replay Worker Playbook" "whoami" '[]' "alert-delivery-replay-worker-recorded" "$PLAYBOOK_RUN_ID")"
...
create_json="$(post_json "/v1/automations/create" "$(
  jq -cn --arg tenant "$TENANT_ID" --arg scope "$SCOPE" --arg automation_id "$AUTOMATION_ID" --arg playbook_id "$PLAYBOOK_ID" \
    '{
      tenant_id:$tenant,
      scope:$scope,
      actor:"automation_alert_delivery_replay_worker_smoke",
      automation_id:$automation_id,
      name:"Automation Alert Delivery Replay Worker Smoke",
      status:"active",
...
echo
echo "== assign owner and sla to replay backlog =="
...
assign_json="$(post_admin_json "/v1/admin/control/alerts/deliveries/assign" "$(
  jq -cn --arg id "$FAILED_DELIVERY_ID" \
    '{ids:[$id],owner:"ops-oncall",escalation_owner:"eng-oncall",sla_target_at:"2026-03-08T09:00:00.000Z",note:"worker replay backlog"}'
)")"
echo "$assign_json" | jq '{ok, updated}'

echo
echo "== run hosted replay worker once =="
worker_json="$(AIONIS_BASE_URL="$BASE_URL" AIONIS_ADMIN_TOKEN="$ADMIN_TOKEN" npm run -s job:hosted-alert-delivery-replay -- --once --tenant-id "$TENANT_ID" --owner ops-oncall --backlog replay_backlog --override-target "$RECOVERY_TARGET" --limit 5)"
echo "$worker_json" | jq '{ok, matched, replayed, failed, skipped, ids}'
worker_replayed="$(echo "$worker_json" | jq -r '.replayed')"
if [[ "$worker_replayed" != "1" ]]; then
  echo "expected hosted replay worker to replay one failed delivery" >&2
  exit 1
fi

echo
echo "== deliveries after worker replay =="
...
if [[ "$replayed_sent_count" != "1" ]]; then
  echo "expected one sent replay row after hosted replay worker" >&2
  exit 1
fi

echo
echo "ok: automation_alert_delivery_replay_worker_smoke completed"
