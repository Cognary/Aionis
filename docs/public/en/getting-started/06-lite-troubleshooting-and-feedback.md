...
4. `lite_write_store` present
5. `lite_recall_store` present

If `memory_store_backend` is not `lite_sqlite`, you are not actually running the Lite runtime you think you are.
...
2. `/v1/automations/*` is server-only
3. Lite is not the recommended production default
4. Lite is not full Server parity

If those route groups return `501 server_only_in_lite`, that is correct Lite behavior.
