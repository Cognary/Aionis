...
```bash
TAG=standalone-v0.2.17 npm run docker:publish:standalone:ghcr:dry-run
```
...
```bash
git tag docker-v0.2.17
git push origin docker-v0.2.17
```

Standalone lane auto trigger:

```bash
git tag standalone-v0.2.17
git push origin standalone-v0.2.17
```
...
```bash
docker pull ghcr.io/cognary/aionis:standalone-v0.2.17
docker pull ghcr.io/cognary/aionis:standalone-latest
docker manifest inspect ghcr.io/cognary/aionis:standalone-v0.2.17 | head
```
