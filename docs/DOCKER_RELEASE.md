...
```bash
TAG=standalone-v0.2.8 npm run docker:publish:standalone:ghcr:dry-run
```
...
```bash
git tag docker-v0.2.8
git push origin docker-v0.2.8
```

Standalone lane auto trigger:

```bash
git tag standalone-v0.2.8
git push origin standalone-v0.2.8
```
...
```bash
docker pull ghcr.io/cognary/aionis:v0.2.8
docker pull ghcr.io/cognary/aionis:latest
docker manifest inspect ghcr.io/cognary/aionis:v0.2.8 | head
```

Standalone image:

```bash
docker pull ghcr.io/cognary/aionis:standalone-v0.2.8
docker pull ghcr.io/cognary/aionis:standalone-latest
docker manifest inspect ghcr.io/cognary/aionis:standalone-v0.2.8 | head
```
