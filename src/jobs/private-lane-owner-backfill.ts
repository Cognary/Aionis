...
async function run() {
  ...
  const out = await withTx(db, async (client) => {
    ...
    const sampleRes = await client.query<{
      ...
      `
      WITH target AS (
        SELECT id, type::text AS type, producer_agent_id
        FROM memory_nodes
        WHERE scope = $1
          AND memory_lane = 'private'
          AND owner_agent_id IS NULL
    ...
    const byTypeRes = await client.query<{ type: string; n: string }>(
      `
      SELECT type::text AS type, count(*)::text AS n
      FROM memory_nodes
      WHERE scope = $1
        AND memory_lane = 'private'
        AND owner_agent_id IS NULL
    ...
    const unmatchedByType = byTypeRes.rows.map((r) => ({ type: r.type, count: Number(r.n) }));
    ...
    if (dryRun || planned.matched === 0) {
      ...
      return {
        ...
        remaining_unfixed_by_type: unmatchedByType,
    ...
    const updRes = await client.query<{ id: string; type: string; action: "set_owner" | "move_shared" }>(
      `
      WITH target AS (
        SELECT id, type::text AS type, producer_agent_id
        FROM memory_nodes
      ...
      RETURNING n.id, n.type::text AS type, p.action
      `,
      [scope, limit, defaultOwnerAgent, defaultOwnerTeam, sharedFallback],
    );
    ...
    const updatedByTypeMap = new Map<string, number>();
    for (const row of updRes.rows) updatedByTypeMap.set(row.type, (updatedByTypeMap.get(row.type) ?? 0) + 1);
    const updatedByType = [...updatedByTypeMap.entries()]
    ...
    const inputSha = sha256Hex(
      `job:private_lane_owner_backfill:${scope}:${startedAt}:${limit}:${String(sharedFallback)}:${defaultOwnerAgent ?? ""}:${defaultOwnerTeam ?? ""}`,
    );

    const diff = {
      job: "private_lane_owner_backfill",
      params: {
        scope,
        limit,
        shared_fallback: sharedFallback,
    ...
    const commitHash = sha256Hex(stableStringify({ parentHash, inputSha, diffSha, scope, actor: "job", kind: "private_lane_owner_backfill" }));
