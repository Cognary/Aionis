...
type FixtureRecord = {
  key: string;
  kind?: "L1_evidence" | "L1_fact" | "L2_topic";
  title: string;
  text_summary: string;
};
...
type ArmName = "l0_plus_l1" | "l0_plus_l1_plus_l2";
...
function nodeId(scope: string, key: string): string {
  return stableUuid(`${scope}:l2-topic-aggregation:${key}`);
}

function edgeId(scope: string, src: string, dst: string): string {
  return stableUuid(`${scope}:l2-topic-aggregation:edge:${src}:${dst}`);
}
...
function makePayload(fixture: Fixture, scope: string, tenantId: string) {
  ...
  const events = eventRecords.map((record) => ({
    ...
    raw_ref: `seed://l2-topic-aggregation/${record.key}`,
    slots: {
  ...
  const edges = [
    ...
    ...l2Nodes.flatMap((node) =>
      [...events, ...l1Nodes].map((source) => ({
        id: edgeId(scope, node.id, source.id),
        scope,
        type: "derived_from",
        src: { id: node.id },
        dst: { id: source.id },
  ...
  return {
    ...
    auto_embed: false,
    memory_lane: "shared",
    input_text: "l2 topic aggregation benchmark seed corpus",
    nodes: [...events, ...l1Nodes, ...l2Nodes],
    edges,
  };
}
...
async function main() {
  ...
  const fixtureFile = path.resolve(argValue("--fixture-file") ?? path.join(root, "src/jobs/fixtures/l2-topic-aggregation-click-v1.json"));
  const tenantId = argValue("--tenant-id") ?? "default";
  const scope = argValue("--scope") ?? `l2_topic_aggregation_${nowTag()}`;
  const outputDir = path.resolve(argValue("--output-dir") ?? path.join(root, "artifacts/benchmarks/l2-topic-aggregation", nowTag()));
  ...
  for (const [arm, allowedLayers] of Object.entries(arms) as Array<[ArmName, string[]]>) {
    for (const item of fixture.cases) {
      ...
      const recallOut = await postJson(baseUrl, "/v1/memory/recall_text", { ...common, return_debug: true });
      const planningOut = await postJson(baseUrl, "/v1/memory/planning/context", {
        ...common,
        tool_candidates: ["rg", "pytest"],
        context: { intent: "analysis", repo: "benchmark" },
      });
