...
async function main() {
  ...
  if (!ctxFile) {
    throw new Error("usage: tsx src/dev/sdk-tools-select.ts <planner_context.json> [candidate ...]");
  }
  const candidateArgs = process.argv.slice(3);
  const candidates = candidateArgs.length > 0 ? candidateArgs : ["psql", "curl", "bash", "rm"];
  ...
  const out = await client.toolsSelect({
    ...(scope ? { scope } : {}),
    context: contextObj,
    candidates,
    include_shadow: true,
