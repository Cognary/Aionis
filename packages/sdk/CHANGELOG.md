...
1. Aligned the TypeScript SDK release line to `0.2.19` for the current coordinated release batch.
...
1. Aligned TypeScript SDK release baseline from `0.2.9` to `0.2.10` after npm registry immutable-version conflict on `0.2.9`.
2. No TypeScript SDK API surface change in this patch release.
...
1. Added replay closed-loop learning projection request/response typing (`learning_projection`, `learning_projection_result`) and warning coverage.
2. Added write guardrail typing support (`MemoryWriteWarning`, `MemoryWriteResponse.warnings`) for commit-only write detection (`write_no_nodes`).
3. Aligned TypeScript SDK release baseline from `0.2.8` to `0.2.9` for cross-channel synchronization.
